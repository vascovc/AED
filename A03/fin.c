...
static void recursive_decoder_1(int encoded_idx,int decoded_idx,int good_decoded_size)
{
	...
	for(int i = 0; i < _c_->n_symbols ; i++)//percorrer os simbolos
	{
        ...
        if(possible == 0)//se o simbolo e possivel
        {
            ...
            {
                _decoded_message_[decoded_idx] = i;//guarda-se na descodificada
                good_decoded_size++;//o good size aumenta
                if(good_decoded_size == _original_message_size_)//se tivermos descodificado tudo
                {
        ...
        while( !stack_empty(&arr) )//tirar tudo do stack e ver se cada simbolo ainda e possivel
        {
            ...
            if(_c_->data[s.symbol].codeword[s.position] != '\0')
            {
                if(_encoded_message_[encoded_idx] == _c_->data[s.symbol].codeword[s.position])
                {
                    ...
                    n_max++;//e uma possiblidade nesta bit da codificada
                }
...
{   
    ...
    {   
        a = q_get(&arr);
        if(_encoded_message_[encoded_idx] == _c_->data[a.symbol].codeword[a.position])//se o bit coincide
        {   
        ...
        else if(_c_->data[a.symbol].codeword[a.position] == '\0')//bit nao coincide;mas e um simbolo possivel do meio da mensagem
...
int main(int argc,char **argv)
{
  ...
  if(argc == 3 && argv[1][0] == '-' && argv[1][1] == 'x')
  {
    ...
    printf("%4d  %8.3f %8.3f %8.3f %8.3f  %4d %6.1f %4d %4d  %3d %3.1f %3.1f %4d\n",n_symbols,t_min,t_avg,t_data[N_OUTLIERS + N_VALID],t_max,u_min,u_avg,u_data[N_OUTLIERS + N_VALID],u_max,m_min,m_avg,m_data[N_OUTLIERS + N_VALID],m_max);
    return 0;
  }
  //
  // Help message
