...
static void recursive_decoder(int encoded_idx,int decoded_idx,int good_decoded_size)
{
	...
	for(int i = 0; i < _c_->n_symbols ; i++)//percorrer os simbolos
	{
        for(int p = 0; (_c_->data[i].codeword[p] != '\0'); p++ )//percorrer o codigo desse simbolo
        {
            ...
            {
                //o codigo do simbolo e diferente logo nao e possivel e passa-se ao proximo simbolo
                break;
            }
        }

        if(encoded_idx_old != encoded_idx)//se o simbolo e possivel, avançou se na mensagem pelo menos 1
        {
            ...
            decoded_idx--;//para o simbolo seguinte tem que se voltar a repor
        }
